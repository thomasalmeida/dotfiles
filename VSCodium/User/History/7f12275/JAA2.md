...
```shell
yay -S hyprland-git waybar-hyprland wofi dunst kitty swaybg swayidle git vim brightnessctl pavucontrol thunar grim slurp network-manager-applet lf neovim cava btop neofetch gedit wget curl pulseaudio brave-bin xdg-desktop-portal-hyprland polkit-gnome qt6-wayland
```
- Copy all file to ``` ~/.config/```

sudo pacman -S pipewire pipewire-media-session pipewire-alsa pipewire-pulse pipewire-jack
